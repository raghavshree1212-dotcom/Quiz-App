...
import { Play, BookOpen, BarChart3, Brain, Loader2, Plus, Zap } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user, onStartQuiz, onNavigateToAdmin }) => {
  ...
  const loadData = async () => {
    ...
    const uniqueTopicsList = Array.from(new Set(allQuestions.map(q => q.topic))).sort();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Welcome back, {user.displayName.split(' ')[0]}!</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
            <Play className="w-5 h-5 text-indigo-600" /> Configure and Start Quiz
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              ...
              <select 
                value={questionCount}
                onChange={(e) => setQuestionCount(e.target.value as any)}
                className="w-full p-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
              >
              ...
              <p className="text-xs text-slate-400 mt-1">Available Qs: {selectedTopic === 'Random (All Topics)' ? stats.totalQuestions : 0 /* approximate for demo */}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
             <button 
              ...
              >
                <div className="font-bold text-amber-500">★</div> Start Bookmarked Review ({stats.bookmarksCount} Qs)
              </button>
          </div>
        </div>
